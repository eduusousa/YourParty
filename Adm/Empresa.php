...
<head>
    ...
    <title>Your Party - Empresa</title>
    ...
    <div class="container">
        ...
        <div class="main">
            ...
            <div class="details">
                ...
                <h2>Maior número de Orçamentos por Serviço</h2>
...
<table>
    ...
    <tbody>
        <?php  
         $stmt5 = $pdo->prepare("   SELECT COUNT(idItemOrcamento),nomeBuffet FROM tbitemorcamento 
                                    INNER JOIN tbbuffet 
                                    on tbbuffet.idBuffet = tbitemorcamento.idBuffet 
                                    GROUP BY nomeBuffet;");	
         $stmt5 ->execute();
...
<table>
    ...
    <tbody>
        <?php  
         $stmt6 = $pdo->prepare("   SELECT COUNT(idItemOrcamento),nomeDecoracao FROM tbitemorcamento 
                                    INNER JOIN tbdecoracao 
                                    on tbdecoracao.idDecoracao = tbitemorcamento.idDecoracao 
                                    GROUP BY nomeDecoracao;");	
         $stmt6 ->execute();
...
<table>
    ...
    <tbody>
        <?php  
         $stmt7 = $pdo->prepare("   SELECT COUNT(idItemOrcamento),nomeLocal FROM tbitemorcamento 
                                    INNER JOIN tbLocal 
                                    on tblocal.idLocal = tbitemorcamento.idlocal 
                                    GROUP BY nomeLocal;");	
         $stmt7 ->execute();
...
<table>
    ...
    <tbody>
        <?php  
         $stmt8 = $pdo->prepare(" SELECT COUNT(idItemOrcamento),descSeguranca FROM tbitemorcamento 
         INNER JOIN tbseguranca 
         on tbseguranca.idSeguranca = tbitemorcamento.idSeguranca 
         GROUP BY descSeguranca  ;");	
         $stmt8 ->execute();
         //$row8 = $stmt8 ->fetch(PDO::FETCH_NUM);
                    ...
                    <?php 
                        ...
                        while($row5 = $stmt5 ->fetch(PDO::FETCH_BOTH)){
                            echo ('<tr>');
                                ...
                                echo '<td>';
                                    echo '<b><h4>Empresas: </b>'. $row5[0]. '<br> <b><span>Empresas: </b>'. $row5[1].'</span></h4>';
                                echo '</td>';
                            echo ('</tr>');
						}	
                    ?>
